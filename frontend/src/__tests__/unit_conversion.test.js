import { parseGenToWei, formatWeiToGen, sanitizeGenInput, toWeiString, weiFromField, minBigInt } from '../money.js';
...
function runUnitConversionTests() {
  ...
  assert(stewardWei === 123456000000000000000n, `Steward decimal failed: got ${stewardWei}`);
  assert(formatWeiToGen(stewardWei) === '123.456', `Steward format failed`);
  ...
  assert(formatWeiToGen(1.5) === '0', 'IEEE-754 number must not be accepted as wei');
  assert(sanitizeGenInput('12.3abc4') === '12.34', 'sanitize strips letters');
  assert(toWeiString(100n) === '100', 'bigint to wei string');
  assert(toWeiString(1.2) === '0', 'JS number rejected as wei string');
  assert(weiFromField('150') === 150n, 'weiFromField integer string');

  const pool = 20n;
  const bonus = 50n;
  const capped = minBigInt(bonus, pool);
  assert(capped === 20n, 'bonus must cap by integer min, never a percentage');
  assert(100n + capped === 120n, 'overturned payout = deposit + capped bonus');
